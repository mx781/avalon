...
from datagen.godot_env import TrainingGodotGoalEvaluator
...
def create_base_benchmark_config(
    data_dir: str = str(uuid.uuid4()), random_int: int = 0, num_frames_alive_after_food_is_gone=20
) -> AvalonSimSpec:
...
def task_groups_from_training_protocol(
    ...
    if training_protocol == TrainingProtocolChoice.MULTI_TASK_BASIC:
        ...
        assert is_meta_curriculum_used
...
class GodotEnvironmentParams(EnvironmentParams):
    ...
    num_frames_alive_after_food_is_gone: int = 10
    level_output_base_path: str = "/mnt/private/data/level_gen"
    ...
    energy_cost_aggregator: Literal["sum", "max"] = "max"
...
class AvalonGodotEnvWrapper(GodotEnv[AvalonObservationType, VRActionType]):
    def __init__(self, params: GodotEnvironmentParams):
        ...
        goal_evaluator = TrainingGodotGoalEvaluator(
    ...
    def curriculum_save_path(self) -> Path:
        ...
        return self.curriculum_base_path / f"{self.params.seed}.pkl"
...
def _transform_observation_rgbd(x: NDArray) -> NDArray:
    # flip y
    x = x[::-1]
    # torch ordering
