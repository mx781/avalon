...
from agent.dreamer.params import Params
    ...
    def load_model(cls, model: torch.nn.Module, device: torch.device) -> None:
        ...
        if cls.model_path.exists():
            try:
                ...
                print(f"exception loading model in worker: {e}")
        else:
...
class DiskStorage(TrajectoryStorage):
    def __init__(
        ...
        self.recent_eps: collections.deque[List[StepData]] = collections.deque(maxlen=20)
    ...
    def log_stats(self) -> None:
        ...
        if not self.wandb:
            return
        if self.log_counter < self.log_freq:
            self.log_counter += 1
            return
        else:
            self.log_counter = 0
        ...
        if self.params.env_params.suite == "godot":
            ...
            for task, x in successes.items():
                for field, y in x.items():
                    log_scalar_mp(self.wandb, f"rollout/{task}/{field}", np.mean(y))
                log_scalar_mp(self.wandb, f"rollout/{task}/num_episodes", len(y))

        log_scalar_mp(self.wandb, "rollout/buffer_size_timesteps", self.timesteps_in_buffer)
        log_scalar_mp(self.wandb, "rollout/total_timesteps", self.total_timesteps)
        log_scalar_mp(self.wandb, "true_buffer_size", measure_buffer_size(str(self.data_dir)))
        ...
        for key, v in samples.items():
            ...
            if v.done:
                ...
                if len(ep) < self.params.min_fragment_len and self.discard_short_eps:
                    print(f"not adding short ep of len {ep_len}")
                    continue
...
class LambdaStorage(TrajectoryStorage):
    ...
    def add_timestep_samples(self, samples: Dict[str, StepData]) -> None:
        for key, trajectory in samples.items():
            self.storage[key].append(trajectory)
