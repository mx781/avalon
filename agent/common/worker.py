...
from agent.common.storage import DiskStorage
...
from agent.common.types import Action
...
from agent.common.types import Info
from agent.common.types import Observation
from agent.common.types import StepData
from agent.common.util import postprocess_uint8_to_float
from agent.common.wandb_lib import log_scalar_mp
...
class AsyncRolloutManager:
    ...
    def start(self) -> None:
        ps = self.multiprocessing_context.Process(
            target=self.safe_entrypoint, args=(self.shutdown_event,), daemon=False
        )
        ps.start()
    ...
    def safe_entrypoint(self, shutdown_event: multiprocessing.synchronize.Event) -> None:
        # We don't wrap this in a loop because it really shouldn't fail flakily.
        # This is just for debugging a fatal crash.
        try:
            self.entrypoint(shutdown_event)
        except KeyboardInterrupt:
            pass
        except Exception as e:
            print("exception in AsyncGamePlayer process")
            print(e)
            print(traceback.format_exc())
            # This basically just gets eaten
            raise

    def entrypoint(self, shutdown_event: multiprocessing.synchronize.Event) -> None:
        torch.set_num_threads(1)
        signal.signal(signal.SIGINT, signal.default_int_handler)
        run_name = self.params.env_params.suite if not self.params.name else self.params.name
        wandb_run = wandb.init(
            name=f"{run_name}-worker_{self.rollout_manager_id}",
        ...
        storage = DiskStorage(self.params, rollout_dir=self.train_rollout_dir, wandb=wandb_run)  # type: ignore
        ...
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    log_scalar_mp(wandb_run, "env_step", self.env_step_counter.value)  # type: ignore
                except KeyboardInterrupt:
                    player.shutdown()
                    storage.shutdown()
                    raise
                except Exception as e:
                    print("caught exception in async worker, will try to reset.")
                    print(e)
                    print(traceback.format_exc())
                    # capture_exception(e)
                    try:
...
class RolloutManager:
    ...
    def run_rollout(
        ...
        exploration_mode: Literal["explore", "eval"] = "explore",
    ):
        """Set num_steps and/or num_episodes to run only that many steps/episodes,
        otherwise if left as None there will be no limit.
        """
        # TODO: make an off-policy optimized version of this method.
        self.model.eval()

        for worker in self.workers:
            worker.remaining_steps = num_steps if num_steps is not None else math.inf
            worker.remaining_episodes = num_episodes if num_episodes is not None else math.inf
    ...
    def run_inference(
        self, exploration_mode: Literal["explore", "eval"]
...
class EnvironmentContainer:
    ...
    def __init__(
        ...
        self.remaining_steps: Union[int, float] = 0  # union to support inf
        self.remaining_episodes: Union[int, float] = 0  # union to support inf
    ...
    def get_step_data(self, actions: Action) -> Tuple[Observation, bool, StepData, str]:
        ...
        if self.remaining_steps == 0 or self.remaining_episodes == 0:
    ...
    def close(self):
        if self.is_multiprocessing:
            try:
                ...
                print("tried to close EnvironmentContainerProcess but pipe was already broken")
            except EOFError:
                print("tried to close EnvironmentContainerProcess but got EOF error.")
        else:
    ...
    def shutdown(self) -> None:
        if self.env:
            try:
                ...
                capture_exception(e)
                print(f"env did not close cleanly due to a TimeoutError: {e}")
                print(traceback.format_exc())
            except Exception as e:
                capture_exception(e)
                print(f"worker failed to shutdown with error {e}")
                print(traceback.format_exc())
