...
from agent.common.types import AlgorithmInferenceExtraInfo
...
from agent.common.worker import StepData
...
from agent.ppo.observation_model import ObservationModel
from agent.ppo.observation_model import PPOModel
        ...
        for k, space in self.action_space.spaces.items():
            if isinstance(space, OneHotMultiDiscrete):
                for act_i in range(len(space.nvec)):
                    for cat_i in range(space.nvec[act_i]):
                        wandb_lib.log_histogram(
                            f"actions/{k}_{act_i}_{cat_i}", batch.action[k][:, act_i, cat_i].float(), i
                        )
                ...
                for act_i in range(space.shape[0]):
                    wandb_lib.log_histogram(f"actions/{k}_{act_i}", batch.action[k][:, act_i].float(), i)
