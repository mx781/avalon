...
parser.add_argument('--task_difficulty_update', default=2e-4, type=float,
...
parser.add_argument('--energy_cost_coefficient', default=1e-4, type=float,
                    help='Coefficient for energy usage')
parser.add_argument('--head_pitch_coefficient', default=1e-4, type=float,
                    help='Coefficient for body kinetic energy')
parser.add_argument('--head_roll_coefficient', default=1e-4, type=float,
                    help='Coefficient for body kinetic energy')
...
parser.add_argument("--is_meta_curriculum_disabled", action="store_true",
                    help="Is meta curriculum disabled during training")
parser.add_argument('--fixed_world_max_difficulty', default=0.5, type=float,
                    help='Fixed world max difficulty')

parser.add_argument("--energy_cost_aggregator", default="max",
...
def main(flags, exit_event: mp.Event):
    ...
    for i in range(flags.num_servers):
        env_config = godot_config_from_flags(flags, random_int=flags.env_seed, env_index=i)
