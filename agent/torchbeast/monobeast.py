...
from agent.torchbeast import atari_wrappers
from agent.torchbeast import avalon_helpers
from agent.torchbeast.atari_wrappers import WarpFrame
from agent.torchbeast.avalon_helpers import FlattenAction
from agent.torchbeast.avalon_helpers import GodotToPyTorch
...
from agent.torchbeast.core import environment
from agent.torchbeast.core import file_writer
from agent.torchbeast.core import prof
from agent.torchbeast.model import DictActionHead
from datagen.godot_env import _randint_of_size
...
def get_flags():
    ...
    flags, argv = parser.parse_known_args(namespace=namespace)
    return flags
        ...
        while True:
            ...
            for t in range(flags.unroll_length):
                ...
                for key in env_output:
                    # print(key)
                    # if key == "action":
                    #     logging.warning(f"action shapes: {buffers[key][index].shape}, {env_output[key].shape}")
...
def learn(
    ...
    with lock:
        ...
        # TODO: explode if action head has learnable parameters
        behavior_policy_dist = model.action_head(batch["policy_logits"])
        target_policy_dist = model.action_head(learner_outputs["policy_logits"])
        action_dict = model.unflatten(batch["action"])
...
def create_buffers(flags, obs_shape, num_policy_logits, flat_action_dim) -> Buffers:
    ...
    specs = dict(
        ...
        policy_logits=dict(size=(T + 1, num_policy_logits), dtype=torch.float32),
        baseline=dict(size=(T + 1,), dtype=torch.float32),
        last_action=dict(size=(T + 1, flat_action_dim), dtype=torch.float32),
        action=dict(size=(T + 1, flat_action_dim), dtype=torch.float32),
    )
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    buffers = create_buffers(
        flags, env.observation_space.shape, model.num_actions, gym.spaces.utils.flatdim(env.action_space)
    )
    ...
    learner_model = Net(env.observation_space.shape, env.action_space, flags.use_lstm).to(device=flags.device)
...
class AtariNet(nn.Module):
    def __init__(self, observation_shape, action_space: spaces.Space, use_lstm=False):
        ...
        num_actions = self.action_head.num_inputs
        ...
        core_output_size = self.fc.out_features + self.last_action_dim + 1
    ...
    def forward(self, inputs, core_state=()):
        ...
        one_hot_last_action = inputs["last_action"].view(T * B, self.last_action_dim)
        ...
        flat_action = self.flatten(action)

        return (
            dict(policy_logits=policy_logits, baseline=baseline, action=flat_action),
            core_state,
        )
...
def create_env(flags):
    if flags.env == "godot":
        ...
        # env = CurriculumWrapper(env, update_rate=args.difficulty_update_rate, std=args.difficulty_std)
        return env
    else:
