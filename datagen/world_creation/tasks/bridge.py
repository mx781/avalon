...
def create_bridge_obstacle(
    ...
    if constraint is None:
        ...
        height_at_location = world.map.Z[world.map.point_to_index(to_2d_point(spawn_location))]
        spawn_location[1] = height_at_location + HALF_AGENT_HEIGHT_VECTOR[1] * 1.1

        # fix up the spawn if too close to the log
        log = world.items[-1]
        assert isinstance(log, Log), "Huh, only expected a single item to be spawned in this case..."
