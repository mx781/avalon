...
def flatten_places_under_items(world: World, earlier_item_count: int, filter_to_classes):
    new_stones = []
    ...
    for i, stone in enumerate(new_stones):
        ...
        if len(distances) > 0 and min(distances) > ITEM_FLATTEN_RADIUS:
...
class StackTaskGenerator(BuildingTaskGenerator):
    ...
    def get_principal_obstacle_params(self, rand: np.random.Generator, difficulty: float, building: Building) -> Tuple:
        ...
        permitted_tiles = list(zip(*(tile_zs, tile_xs)))
        spawn_room, spawn_tile = decide_spawn_room_and_tile(rand, first_story, {room.id: permitted_tiles})
