...
from datagen.world_creation.indoor.components import FloorChasm
...
class JumpTaskGenerator(BuildingTaskGenerator):
    ...
    def get_principal_obstacle_params(self, rand: np.random.Generator, difficulty: float, building: Building) -> Tuple:
        ...
        spawn_tile = rand.choice([BuildingTile(i, 0) for i in range(spawn_room.width)])
        target_tile = rand.choice([BuildingTile(i, spawn_room.length - 2) for i in range(1, spawn_room.width - 1)])
