...
from datagen.world_creation.indoor.tests.helpers import make_tarball
...
def generate_reference_worlds(export_path_base: Path):
    for world_name, params in INDOOR_WORLD_CATALOG.items():
        logger.info(f"Generating reference world {world_name}")
        output_path = export_path_base / world_name
        output_path.mkdir()

        make_indoor_world(*params, output_path=output_path)
...
def make_checksum_json(source_path: Path, snapshot_commit: str, json_output_path: Path) -> None:
    checksums = dict(snapshot_commit=snapshot_commit, checksums={})
...
def main(args: List[str]):
    ...
    if not is_git_repo_clean_on_notebook():
        logger.warning(
    ...
    references = [
        ...
        DataReference("indoor_worlds", generate_reference_worlds, "indoor_world*"),
    ]
