...
from datagen.world_creation.indoor.constants import LADDER_THICKNESS
...
class RoomTileGraph(TileGraph):
    ...
    def find_shortest_path(
        ...
        avoid_points = sum([list(room.get_tile_positions(buffer=1)) for room in avoid_rooms], [])
        graph_copy.remove_nodes_from(avoid_points)
        return nx.astar_path(graph_copy, (point_a.z, point_a.x), (point_b.z, point_b.x))
