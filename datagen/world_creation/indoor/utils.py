...
from datagen.world_creation.indoor.wall_footprint import WallFootprint
...
def rand_integer(rand: np.random.Generator, min_ge: int, max_lt: int):
    if min_ge >= max_lt:
        return min_ge
    else:
...
def inset_borders(array: np.ndarray, min_leftover_size: int = 0, void_value: Any = 1, border_value: Any = 0):
    """
    ...
    convolution = scipy.ndimage.convolve(array.astype(np.int), np.full((3, 3), void_value), mode="constant")
