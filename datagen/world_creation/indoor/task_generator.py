...
from datagen.world_creation.configs.biome import generate_biome_config
...
from datagen.world_creation.indoor.doors import get_door_params_from_difficulty
from datagen.world_creation.indoor.doors import make_entrance_hinge_door
from datagen.world_creation.indoor.doors import make_entrance_sliding_door
from datagen.world_creation.types import Point3DNP
from datagen.world_creation.worlds.world import World
from datagen.world_creation.worlds.world import _get_spawn
from datagen.world_creation.worlds.world import build_building

EntranceSite = Tuple[int, int, Tuple[Azimuth, ...]]
IndoorTaskParams = Tuple[Building, List[Entity], Point3DNP, Point3DNP, Tuple[EntranceSite, ...]]
...
class BuildingTaskGenerator:
    ...
    def add_entrance_obstacles(
        ...
        if door_type is HingeDoor:
            ...
            make_entrance_door = make_entrance_sliding_door
        else:
...
def decide_spawn_room_and_tile(
    ...
    if entrances := story.entrances:
        ...
        if permitted_tiles_by_room_id is not None:
            ...
            spawn_tile = tuple(rand.choice(permitted_tiles_by_room_id[spawn_room_id]))
        else:
