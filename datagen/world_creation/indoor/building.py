...
from datagen.world_creation.indoor.blocks import WindowBlock
from datagen.world_creation.indoor.blocks import make_blocks_from_tiles
from datagen.world_creation.indoor.blocks import new_make_block_nodes
...
from datagen.world_creation.indoor.components import set_room_tiles_in_grid
...
from datagen.world_creation.indoor.constants import EXPORT_MODE
from datagen.world_creation.indoor.constants import HIGH_POLY
from datagen.world_creation.indoor.constants import Azimuth
from datagen.world_creation.indoor.constants import ExportMode
...
from datagen.world_creation.indoor.mesh import make_pretty_building_meshes
...
from datagen.world_creation.utils import make_transform
...
class Building:
    ...
    def width(self):
        ...
        """
        return first(self.stories).width

    @property
    def length(self):
        return first(self.stories).length

    @property
    def offset_point(self) -> Point3DNP:
        # Where to offset any building space coordinates from to get the global ones
        return self.position - np.array([self.width / 2, 0, self.length / 2])
    ...
    def get_footprint_outline(self, story_num: int = 0):
        tile_size = 1
        ...
        for story_num, blocks in blocks_by_story.items():
            ...
            for i, block in enumerate(blocks):
                ...
                story_block_nodes = new_make_block_nodes(
                    scene,
                    block_in_building_space.centroid,
                    size=block_in_building_space.size,
        ...
        for story_link_num, story_link in enumerate(self.story_links):
            ...
            for i, level_block in enumerate(story_link_blocks):
                ...
                story_link_block_nodes = new_make_block_nodes(
                    scene,
                    translated_block.centroid,
                    size=translated_block.size,
    ...
    def _generate_interior_wall_blocks(self, story_num: int) -> List[LevelBlock]:
        ...
        for block in interior_wall_blocks:
            stretched_block = WallBlock(
                block.x,
                IntRange(-story.floor_negative_depth, story.outer_height - CEILING_THICKNESS),
                block.z,
                is_interior=True,
            )
    ...
    def _generate_exterior_wall_blocks(self, story_num: int) -> List[LevelBlock]:
        ...
        for block in exterior_wall_blocks:
            offset_block = WallBlock(
                block.x, IntRange(-story.floor_negative_depth, story.outer_height), block.z, is_interior=False
            )
    ...
    def _generate_hallway_floor_blocks(self, story_num: int) -> List[LevelBlock]:
        ...
        return [
            FloorBlock(block.x, IntRange(-story.floor_negative_depth, DEFAULT_FLOOR_THICKNESS), block.z)
            for block in make_blocks_from_tiles(hallway_tiles, solid_tile_value=TileIdentity.HALLWAY.value)
        ]

    def _generate_hallway_ceiling_blocks(self, story_num: int) -> List[LevelBlock]:
        """
        ...
        for height in unique_heights:
            ...
            blocks.extend(
                [
                    CeilingBlock(block.x, IntRange(DEFAULT_FLOOR_THICKNESS + height, story.outer_height), block.z)
    ...
    def _generate_ceiling_blocks(self, story_num: int) -> List[LevelBlock]:
        story = self.stories[story_num]
        footprint = inset_borders(story.footprint).astype(np.int)
        for story_link in story.story_links:
        ...
        return [
            CeilingBlock(x=block.x, y=IntRange(story.outer_height - CEILING_THICKNESS, story.outer_height), z=block.z)
...
class BuildingNavGraph(Graph):
    ...
    def plot(self, origin: Point3DNP = np.array([0, 0, 0]), ax=None):
