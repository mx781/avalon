...
from datagen.world_creation.debug_plots import plot_points
from datagen.world_creation.debug_plots import plot_triangulation
...
class Terrain:
            ...
            for tree_resource, tree_list in tree_list_by_resource.items():
                tree.root.add_child(
                    create_multimesh_instance(
                        ...
                        tree_list,
                        first(tree_list).resource_file.replace(".tscn", ".res"),
            ...
            for tree_resource, tree_list in tree_list_by_resource.items():
                tree.root.add_child(
                    create_multimesh_instance(
                        ...
                        f"high_tree_{tile_x}_{tile_z}",
                        tree_list,
                        first(tree_list).resource_file.replace(".tscn", "_low.res"),
                        self.biome_map.config.flora_config,
                    )
                )
    ...
    def _add_tile_local_flora(
        self, flora_by_tile_ids_and_resource, neighboring_and_this_tile_ids, scene, tile_x, tile_z, tree
    ):
        ...
        for flora_resource, flora_list in flora_list_by_resource.items():
            tree.root.add_child(
                create_multimesh_instance(
                    ...
                    f"high_tree_{tile_x}_{tile_z}",
                    flora_list,
                    first(flora_list).resource_file.replace(".tscn", ".res"),
                    self.biome_map.config.flora_config,
                )
            )
...
def _create_triangulation(
    ...
    for region in special_regions:
        ...
        points = [x for x in points if not region.contains_point_2d(x)] + region.points
    edge_points = np.array([(x[0], x[2]) for x in edge_region.edge_vertices])
    points = np.concatenate((edge_points, np.array(points)))
    if IS_DEBUG_VIS:
        plot_points(points, 0, 1)
...
def _print_array(array: np.ndarray, precision) -> str:
