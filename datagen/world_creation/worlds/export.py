...
from datagen.world_creation.configs.flora import FloraConfig
from datagen.world_creation.constants import IS_ALL_SCENERY_IN_MAIN
from datagen.world_creation.constants import UP_VECTOR
from datagen.world_creation.constants import WATER_LINE
from datagen.world_creation.debug_plots import plot_value_grid_multi_marker
...
from datagen.world_creation.entities.food import Food
...
from datagen.world_creation.entities.tools.boulder import Boulder
from datagen.world_creation.entities.tools.colored_sphere import ColoredSphere
...
from datagen.world_creation.worlds.biome_map import make_fast_biome
from datagen.world_creation.worlds.height_map import clamp
...
from datagen.world_creation.worlds.terrain import _create_static_body
from datagen.world_creation.worlds.terrain import create_multimesh_instance
from datagen.world_creation.worlds.world import World
from datagen.world_creation.worlds.world import get_spawn
...
def _create_visualization_world(output_folder: Path, world: World) -> World:
    ...
    food_items = [x for x in world.items if isinstance(x, Food)]
    if len(food_items) == 0:
    ...
    if world.export_config.debug_visualization_config.is_2d_graph_drawn:
        marker_lists = []
        for type_name, points in points_by_type.items():
            color = color_by_type_name[type_name]
...
def _get_pitch_and_yaw_from_sky_params(sky_config: Dict[str, Any]):
    pitch = (180.0 - sky_config.get("sun_latitude", 170.0)) * -1.0
    yaw = sky_config.get("sun_longitude", 0.0) * -1.0
    return pitch, yaw


def _get_transform_from_pitch_and_yaw(pitch, yaw):
...
def _create_tree_shape(
    scene: GodotScene, tree_obj, i, j, flora_config: Dict[str, FloraConfig], resource: str
) -> GDNode:
