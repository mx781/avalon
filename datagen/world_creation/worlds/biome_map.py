...
from datagen.world_creation.constants import DARK_SHORE
from datagen.world_creation.constants import FRESH_WATER
from datagen.world_creation.constants import MAX_CLIFF_TERRAIN_ANGLE
from datagen.world_creation.constants import MIN_CLIFF_TERRAIN_ANGLE
from datagen.world_creation.constants import SWAMP
from datagen.world_creation.constants import UNCLIMBABLE_BIOME_ID
...
from datagen.world_creation.types import Biome
from datagen.world_creation.types import FloatListNP
from datagen.world_creation.types import MapBoolNP
from datagen.world_creation.types import MapFloatNP
from datagen.world_creation.types import MapIntNP
from datagen.world_creation.types import Point2DListNP
from datagen.world_creation.types import Point3DListNP
from datagen.world_creation.types import SceneryBorderMode
from datagen.world_creation.utils import hex_to_rgb
...
from datagen.world_creation.worlds.height_map import create_kd_tree
from datagen.world_creation.worlds.height_map import get_border_points
from datagen.world_creation.worlds.height_map import get_flora_config_by_file
...
class BiomeMap:
    ...
    def get_colors(
        ...
        return np.clip(colors + rand.normal(0.0, jitters, colors.shape), 0.0, 1.0)
    ...
    def get_random_points(
        ...
        points_2d = np.zeros_like(selected_cells, dtype=np.float)
...
def rotation_matrix_from_vectors(b):
    a = UP_VECTOR
    v = np.cross(a, b)
    c = np.dot(a, b)
    s = np.linalg.norm(v)
...
def make_fast_biome(map: HeightMap, natural_biome_config: BiomeConfig):
    ...
    biome_id = np.ones(map.Z.shape, dtype=np.int)
    coast_biome_id = 1
    biomes = tuple(
        [
...
def plot_biome_grid(data, config: BiomeConfig, title=""):
    data = np.array(data)
    fig, ax = plt.subplots()
    fig.set_size_inches(8, 8)
    if title:
