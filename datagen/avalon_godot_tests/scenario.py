...
from datagen.env_helper import observation_video_tensor
...
from datagen.world_creation.tasks.eat import add_food_and_tree
...
from datagen.world_creation.worlds.height_map import Point3DNP
from datagen.world_creation.worlds.height_map import _LegacyEdgeNoise
...
class Scenario:
    ...
    def look_at(cls, name: str, distance: float):
        return DebugCameraAction.isometric(f"{name}__1", distance=distance)
    ...
    def observe(self, env: AvalonEnv, output_path: Path) -> List[AvalonObservationType]:
        ...
        env.episode_tracker.episode_count = 0
        env.reset_nicely_with_specific_world(episode_seed=0, world_path=str(world_file))
        observations = []
    ...
    def run(self, env: AvalonEnv, output_path: Path) -> ScenarioObservations:
        ...
        debug_output = []
        with open(debug_json_path) as line_delimited_debug_json:
            failures = []
            size = 0
            for l, frame_dict in enumerate(line_delimited_debug_json):
                try:
                    debug_output.append(json.loads(frame_dict))
                except json.JSONDecodeError as e:
                    print(f"Failed to decode debug log line {l} {frame_dict}")
                    failures.append(frame_dict)
                size = l
            if len(failures) > 0:
                print(f"{len(failures)}/{size} janked up log lines in {debug_json_path}")
        ...
        # TODO: this is the piece that's broken now
        # shutil.move(folder, os.path.join(env.config.dir_root, self.name))
        # shutil.move(scene_path, os.path.join("/home/user", self.name))
        return obs
...
def _facing_2d(from_point: Point3DNP, to_point: Point3DNP) -> np.ndarray:
    ...
    return Rotation.from_euler("y", yaw).as_matrix().flatten()
